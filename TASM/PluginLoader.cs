...
public class PluginLoader
{
    static void LoadPluginFromAssembly(string assemblyPath)
    {
        ...
        {
            Logging.Log(Logging.LogLevel.Info, "PluginLoader", $"Found no GamePlugin metadata in {friendlyName}, skipping.");
            return;
        }
        
        Logging.Log(Logging.LogLevel.Info, "PluginLoader", $"Loading Plugin: {attribute.Name} ({attribute.Version}) by {attribute.Author}");
    ...
    public static void LoadAllPlugins()
    {
        ...
        foreach (var assemblyFile in Directory.EnumerateFiles(Paths.TasmPluginsRoot!, "*.dll", SearchOption.AllDirectories))
        {
